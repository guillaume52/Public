...
token='token between the quotes'
headers = {'Authorization': 'Bearer {}'.format(token)}
#create a dataframe that can de downloaded as a temaplte
...
#Open excel file
def openExcel():
    files=fileopenbox("Upload Excel file", "Optimizely", filetypes= "*.xlsx", multiple=False)
    ...
    try:
        df = df.convert_dtypes() #convert columns to their likely types - this will fail in more than one type found in one colum e.g int and string, default is convert 
        print("converted")
    except:
        df=df.astype(str)
...
def list_audiences(project_id,headers):
    ...
    Audience=r_total_json  #store the JSON containg all the audiences
    
    for P in Audience: # loop through JSON and read sepecific values
        audience_value=[]
        ...
        Audience_List.append({
           ...
           'Audience Stream ID':re.sub(re.sub("[\d]+","",audience_value),"",audience_value), #audience_value is read directly from optimizely to           
...
def prioritisation():
    ...
    lists = []
    # make list of lists
    for i in sorted(QA_df['Experiment ID']):
    # append list
        lists.append([i])
    # Display result
    print(lists)
...
def QA(Campaign_id):  
    ...
    #merge_list=["Experiment Name","Audience name","Variation Name","AEM image","Project ID","Campaign id","Experiment ID","Variation ID","Audience ID","Audience Stream ID"]
    ...
    #for QAindex, QA_row in QA_done[0:1].iterrows(): 
        ...
        if pd.isna(QA_row['page_id_Optimizely']):
            for j in range(0,len(merge_list)):
                ...
                QA_check=[]
                #QA_check=pd.merge(df[df['Experiment Name']==QA_row['Experiment Name']].astype(str),QA_df.astype(str),on=np.asarray(merge_list)[[merge_list_append]].tolist() , how = 'left')            
                ...
                #if str(QA_check['page_id'].item()) == "nan":   
                #if pd.isna(QA_check['page_id']):
...
def upload_experiments(df):
        ...
        for index, row in df[start_index:end_index].iterrows():
            ...
            Extensions_id=str(int(row['Extension_Id']))
            ...
            try:
                if '_merge' and 'Project ID_y' and 'Experiment ID_y'  in df:
                    ...
                    elif row['Experiment ID_y'].isdigit() and 'both' not in row['_merge']: # Optimizely doesnt allow updating Perzonalisation, only AB test, so the below wont work
                    ...
                    r.raise_for_status()
                    Experiments=r.json()
                    #This collets all the info about the newly created experience
        ...
        ##check that all expeirment have been created succesfully
        #if all(a == 'Experiment created succesfully' for a in df['Upload Status']):
          #  print("True")
...
if Menu_selection =="New Campaign":
    ...
    #df.to_excel( os.path.join(path,'HP_AEM_checked.xlsx'),sheet_name= 'data_checked' )
    ...
    #df[~df['Experiment Name'].isin(QA_df['Experiment Name'])]
...
elif Menu_selection =="Check for External links":######this is not working, we need to be able to pull all type of test n ot just personalisation
